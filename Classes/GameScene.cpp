...
bool GameScene::init()
{
	...
	for (int i = 0; i <= 30; i++)
	{
		draw->drawSegment(Vec2(i * 20, 0), Vec2(i * 20, 600), 1, Color4F(0.5, 0.5, 0.5, 1));
	}
	for (int i = 0; i <= 30; i++)
	{
		draw->drawSegment(Vec2(0, i * 20), Vec2(600, i * 20), 1, Color4F(0.5, 0.5, 0.5, 1));
	}
	addChild(draw);

	/*
	auto menuItemBack = MenuItemFont::create("BACK", CC_CALLBACK_1(GameScene::menuCallBack, this));
	auto labelScore = Label::create("SCORE 0");
	...
	*/
	...
	spHead->setPosition((Vec2(rand() % 30, rand() % 30) * 20) + Vec2(10, 10));
	spFood->setPosition((Vec2(rand() % 30, rand() % 30) * 20) + Vec2(10, 10));
	addChild(spHead);
	addChild(spFood);
...
void GameScene::gameLogic(float t)
{
	...
	auto NowX = GameScene::spHead->getPosition().x;
	auto NowY = GameScene::spHead->getPosition().y;
	if (NowX > 600 || NowX < 0 || NowY > 600 || NowY < 0)
	{
		Director::getInstance()->replaceScene(createScene());
	}
